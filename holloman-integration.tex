...
Benchmarks on 1,000 PE files, 1.72 GB show 12.7~ms/file average scan time
...
Each line contains a 34-character cluster_id (X.<32hex>), a size field (0 or * for
wildcard), and a malware name.  The 34-char cluster_id contains the
...
\texttt{CMakeLists.txt (root + libclamav) \CMakeLists.txt} & +12 & Link \texttt{libholloman\_avx2.a}, set \texttt{HAVE\_HOLLOMAN} \\ holloman/CMakeLists.txt \CMakeLists.txt} & +12 & Link \texttt{libholloman\_avx2.a}, set \texttt{HAVE\_HOLLOMAN} \\ Source files built in-tree (no external dep)
\bottomrule
\end{tabular}
...
\item \texttt{source2yara.py} --- Queries ClickHouse for all clusters from a
  ...
  \texttt{holloman.cluster\_id} conditions.
...
The test corpus consisted of 1,000 PE files, 1.72 GB, drawn from
...
\textbf{Average per file} & \textbf{12.7 ms} \\
...
Benchmarks (1,000 files) & 14.2 ms/file, 121.4 MB/s \\
...
\begin{itemize}[nosep]
\item \textbf{14.2 ms/file} average scan time (including disk I/O)
